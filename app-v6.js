...
function renderLibrary(){
  ...
  const filterBox=$('#categoryFilters');
  if(filterBox){
    filterBox.innerHTML=categories.map(cat=>`<button class="category-chip ${cat===activeCategory?'active':''}" data-category="${escapeAttr(cat)}">${escapeHtml(cat)}</button>`).join('');
    filterBox.querySelectorAll('[data-category]').forEach(btn=>btn.onclick=()=>{activeCategory=btn.dataset.category;renderLibrary();});
  }
  const visible=activeCategory==='All'?BOOKS:BOOKS.filter(b=>(b.category||'Other')===activeCategory);
  $('#libraryCount').textContent=`${BOOKS.length} books · ${BOOKS.reduce((n,b)=>n+b.chapters.length,0).toLocaleString()} learning pages`;
  $('#bookGrid').innerHTML = visible.map(book => {
    const p=progress(book);
    return `<article class="book-card">
      <div><div class="book-card-top"><div class="book-icon">${book.symbol}</div><span class="category-badge">${escapeHtml(book.category||'Book')}</span></div><h2>${book.title}</h2><p>${book.description}</p></div>
