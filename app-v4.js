...
function applyPrefs(){
  ...
  $('#aiEndpointInput').value=state.aiEndpoint || (window.BEGINNER_BOOKS_CONFIG?.aiEndpoint || '');
}
...
$('#saveAiEndpoint').onclick=()=>{
  ...
  save();
  $('#aiEndpointStatus').textContent=state.aiEndpoint?'Saved. Ask AI will use this Worker endpoint.':'Cleared. AI tutor is not connected.';
};
...
function effectiveAiEndpoint(){
  const raw=(state.aiEndpoint || window.BEGINNER_BOOKS_CONFIG?.aiEndpoint || '').trim();
...
function openAi(){
  ...
  if(!$('#aiMessages').children.length){
    ...
    else addAiMessage('assistant','AI tutor is ready in the website, but the Cloudflare Worker endpoint has not been connected yet. Open Reading settings → AI tutor endpoint after you deploy the included worker.');
  }
  aiDialog.showModal();
...
$('#aiSetupBtn').onclick=()=>{aiDialog.close();settingsDialog.showModal();setTimeout(()=>$('#aiEndpointInput').focus(),100);};
...
async function sendAi(){
  ...
  input.value='';
  if(!endpoint){ addAiMessage('assistant','Connect the included Cloudflare Worker first. In Reading settings, paste its /api/chat URL into “AI tutor endpoint”.'); return; }
